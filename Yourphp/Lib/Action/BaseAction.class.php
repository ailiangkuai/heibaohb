...
class BaseAction extends Action
{
    ...
    public function index($catid='',$module='')
    {
		...
		if($module=='Guestbook'){
			...
			if($catid){
				...
				$this->assign('firstRow',0);//先初始化，避免下面的count为空使用了未赋值的变量
				$this->assign('listRows',0);
				$this->assign('catids',$cat['arrchildid']);
				$where = " status=1 ";
				if($cat['child']){							
	...
	public function down()
	{
		...
		$this->dao->where("id=".$id)->setInc('downs');//对字段downs进行自增
