...
GDNetMenuDown.spacerGif    = "/Public/Image/bg_1001.gif";
GDNetMenuDown.dingbatOn    = "/Public/Image/expand-1.gif";
GDNetMenuDown.dingbatOff   = "/Public/Image/expand-0.gif";
GDNetMenuDown.dingbatSize  = 14; 
GDNetMenuDown.menuPadding  = 10; 
GDNetMenuDown.itemPadding  = 3; 
GDNetMenuDown.shadowSize   = 2; 
GDNetMenuDown.shadowOffset = 3; 
GDNetMenuDown.shadowColor  = "#888"; 
GDNetMenuDown.backgroundColor = "#D9ECFF"; 
...
GDNetMenuDown.isSupported = function() 
{
  ...
  else if (an == "Microsoft Internet Explorer") {if (document.getElementById) r = true; }
  GDNetMenuDown.isSupported.r = r;
  return r;
}

GDNetMenuDown.initialize = function() {for (var i = 0, menu = null; menu = this.registry[i]; i++) {menu.initialize();}}
...
if (_this.items[this._index].url) {
  var sblank=_this.items[this._index].url.slice(0,2).toLocaleUpperCase()
  if ( URL_blank=='BLANK') { window.open( _this.items[this._index].url); } else { location.href = _this.items[this._index].url; }
 }}
}
...
'<div class="content"><table class="items" cellpadding="0" cellspacing="0" border="0"><tr><td colspan="2"><img src="' + GDNetMenuDown.spacerGif + '" width="1" height="3"></td></tr>' + 
aHtml.join('') + 
...
return '<tr class="item"><td width="4"></td><td nowrap style="' + sPaddingLeft + '">' + 
...
function loadImage(sFilename) {
   var img = new Image();
   img.src ="/Public/Image/" + sFilename;
   return img;
}
